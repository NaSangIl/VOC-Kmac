...
let deleteMstData = function() {
	...
	let msg2 = rowCnt > 0 ? "VOC 처리 유형도 삭제됩니다." : ""; //하위 VOC유형목록에 데이터가 존재할 경우만 표시
	if (!confirm('VOC 처리 유형 그룹 정보를 삭제하시겠습니까? ' + msg2)) return;
				...
				if (0 < list?.length) {
					if (initDataInfo) {
						...
						if (flag == '1') {
							$('#selectCustomCode2').html('');
							$('#selectCustomCode3').html('');
							$('#vocActTypeEditPop .btn-regi-custom1').removeClass("disabled");
							$('#vocActTypeEditPop .btn-regi-custom2').addClass("disabled");
							$('#vocActTypeEditPop .btn-regi-custom3').addClass("disabled");
							searchData('2');
						} else if (flag == '2') {
							$('#vocActTypeEditPop .btn-regi-custom1').removeClass("disabled");
							$('#vocActTypeEditPop .btn-regi-custom2').removeClass("disabled");
							$('#vocActTypeEditPop .btn-regi-custom3').addClass("disabled");
							$('#selectCustomCode3').html('');
							searchData('3');
						}
					}
					$('#vocActTypeEditPop .btn-regi-custom' + flag).removeClass("disabled");
				} else {
					if (flag == '1') {
						resetData('1');
						$('#vocActTypeEditPop .btn-regi-custom1').removeClass("disabled");
						...
						$('#vocActTypeEditPop .btn-regi-custom3').removeClass("disabled");
					}
				}
...
let resetData = function(flag) {
	...
	$('#vocActTypeEditPop').find('input').attr("readonly", true);
	$frm.find('input').attr("readonly", false);
	$('#vocActTypeEditPop [class*="btn-save-custom"]').hide();
	$('#vocActTypeEditPop').find('.btn-save-custom' + flag).show();
	$('#vocActTypeEditPop [class*="btn-delt-custom"]').hide();
}
...
let setData = function(flag, d) {
	...
	$('#vocActTypeEditPop').find('.btn-save-custom' + flag).show();
	$('#vocActTypeEditPop [class*="btn-delt-custom"]').hide();
	$('#vocActTypeEditPop').find('.btn-delt-custom' + flag).show();
}
...
let uploadExclTemplate = function() {
	...
	.done(function(result) {
		...
		if (result && result?.messageCode == '0000') {
			alert("VOC 처리 유형 (대) " + lv1Cnt + " 건, (소) " + lv2Cnt + " 건 등록되었습니다.\n중복 데이터는 " + dupCnt + " 건 발생했습니다.");
			$('#excelFile').val("");
			loadGrid(customMstSeq);
		}else{
